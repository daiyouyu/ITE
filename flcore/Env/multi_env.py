...
class BatteryEnvSingle(gym.Env):
    ...
    def __init__(self,
        ...
        # 锅炉参数
        self.Fbmax = float(Fbmax)
        self.LHV = float(LHV)
        self.cco2 = float(cco2)
        self.cf = float(cf)
        self.gf = float(gf)
        ...
        # 外生量占位（由协调器在每步注入）
        self._exog = {"L": 0.0, "R": 0.0, "P": 0.0, "sin_h": 0.0, "cos_h": 0.0}
    ...
    def set_exogenous(self, L, R, P, sin_h, cos_h, L_scale=1.0, R_scale=1.0, P_scale=1.0):
        self._exog = {"L": float(L), "R": float(R), "P": float(P),
                      "sin_h": float(sin_h), "cos_h": float(cos_h)}
        self._L_scale = max(1e-6, float(L_scale))
    ...
    def step(self, action: np.ndarray):
        # 读取当步外生量（由协调器注入）
        L = self._exog["L"]; R = self._exog["R"]; P = self._exog["P"]
    ...
    def _boiler_block(self,  a_norm: float):
        ...
        u = max(0.0, 2.0 * (u - 0.5))  # 去中心：u=0.5 -> 0，u=1->1，u=0->0
        M_bfw = u * self.Fbmax
        Hfuel = M_bfw * self.LHV
        noise = self._rng.integers(low=-2, high=2) * 0.01
        p_gen = Hfuel / 3600.0 * (0.43 + noise)
        return M_bfw, p_gen
...
class MultiBatteryCoordinator(gym.Env):
    ...
    def __init__(self, series: Dict[str, np.ndarray], n_agents: int, **single_kwargs):
        ...
        # 归一化尺度（统一由协调器计算并下发）
        self._L_scale = max(1.0, float(np.percentile(self.series[0]['L'], 99)))
    ...
    def _inject_exogenous_to_all(self, t: int):
        for i,(aid,env) in enumerate(self.envs.items()):
            L = float(self.series[i]["L"][t])
            R = float(self.series[i]["R"][t])
            P = float(self.series[i]["P"][t])
            sin_h = float(self.series[i]["sin_h"][t])
            cos_h = float(self.series[i]["cos_h"][t])
            env.set_exogenous(L, R, P, sin_h, cos_h,
                                  L_scale=self._L_scale, R_scale=self._R_scale, P_scale=self._P_scale)
    ...
    def step(self, actions: Dict[str, np.ndarray]):
        ...
        for (need_price,buyer_aid, need) in buyers:
            ...
            for (ask, seller_aid, _) in sellers:
                ...
                check = ask if ask > need_price else need_price
                # 成交记录（MWh与现金）
        ...
        for aid in self.agents:
            ...
            # 卖不掉的富余电报废
            offer_MW = float(inf.get("offer_MW", max(0.0, -net)))
            ...
            total_cost = local_cost + elec_cost
