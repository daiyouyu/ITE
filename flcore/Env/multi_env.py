...
class BatteryEnvSingle(gym.Env):
    ...
    def __init__(self,
        ...
        # 锅炉参数
        self.boiler_e = boiler_E
        self.cf = cf
        self.boiler_a = boiler_a
        self.boiler_b = boiler_b
        self.boiler_c = boiler_c
        ...
        self.cco2 = 0.0712
        # 外生量占位（由协调器在每步注入）
        self._exog = {"G_L": 0.0, "H_L": 0.0, "R": 0.0, "P": 0.0, "sin_h": 0.0,
                      "cos_h": 0.0, "R_solar": 0.0, "R_wind": 0.0}
    ...
    def set_exogenous(self, G_L, H_L, R, P, sin_h, cos_h, R_solar=0.0, R_wind=0.0, G_L_scale=1.0, H_L_scale=1.0,
                      R_scale=1.0, P_scale=1.0):
        self._exog = {
            "G_L": float(G_L),
            "H_L": float(H_L),
            "R": float(R),
    ...
    def step(self, action: np.ndarray):
        ...
        R = self._exog["R"];
        P = self._exog["P"]
        R_solar = self._exog["R_solar"]
        R_wind = self._exog["R_wind"]
        ...
        P_boiler_e = self._boiler_block(boiler_a)
        # CHP动作
    ...
    def _boiler_block(self, a_norm: float):
        ...
        p_gen = u * self.boiler_e
        return p_gen

    def _boiler_cost(self, P):
...
class MultiBatteryCoordinator(gym.Env):
    ...
    def _inject_exogenous_to_all(self, t: int):
        for i, (aid, env) in enumerate(self.envs.items()):
            ...
            R_solar = float(self.series[i]["R_solar"][t])
            R_wind = float(self.series[i]["R_wind"][t])
            P = float(self.series[i]["P"][t])
            sin_h = float(self.series[i]["sin_h"][t])
            cos_h = float(self.series[i]["cos_h"][t])
            env.set_exogenous(G_L, H_L, R, P, sin_h, cos_h, R_solar, R_wind,
    ...
    def step(self, actions: Dict[str, np.ndarray]):
        ...
        for aid in self.agents:
            ...
            elec_cost = grid_cost - (market_cash if market_sell_MWh > 0 else 0.0) \
                        + (market_cash if market_buy_MWh > 0 else 0.0)
            # 外网补足热力
